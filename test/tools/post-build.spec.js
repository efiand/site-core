...
describe('tools/post-build', () => {
	...
	before(() => {
		tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'site-core-post-build-'));
		...
		fs.writeFileSync(path.join(tempRoot, 'docs', 'migration.md'), '# Migration\n');
	});
	...
	test('готовит dist для GitHub Pages', async () => {
		...
		const migrationMd = fs.readFileSync(path.join(tempRoot, 'dist', 'docs', 'migration.md'), 'utf8');
		assert.match(migrationMd, /^---\n/);
		assert.match(migrationMd, /# Migration/);
	});
});
